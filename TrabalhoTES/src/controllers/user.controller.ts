...
export class UserController {
  ...
  async deletarUsuarioPorNome(request: Request, response: Response): Promise<Response> {
    ...
    try {
      console.log("Iniciando exclusão do usuário:", nome);
      ...
      console.log("Usuários encontrados para exclusão:", users);
  
      if (users.length === 0) {
        console.log("Nenhum usuário encontrado para exclusão.");
        return response.status(404).json({ message: "Usuário não encontrado" });
      }
      ...
      console.log("Operações de exclusão concluídas com sucesso.");
  
      return response.status(200).json({ message: "Usuário(s) deletado(s) com sucesso" });
    } catch (error) {
      console.error("Erro ao excluir o usuário:", error);
      return response.status(500).json({ error: 'Erro ao deletar o(s) usuário(s)' });
    } finally {
      await prisma.$disconnect();
    }
  }
